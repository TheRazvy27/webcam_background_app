...
DEFAULT_BLUR = 3
DEFAULT_MORPH = 1
DEFAULT_FACE_SCALE = 145  # percent
DEFAULT_MOUTH = 1


def default_model_path():
    """!Return default segmentation model path if present.
...
def default_face_landmarker_path():
    """!Return default face landmarker model path if present.

    @return Path string or None.
    """
    candidates = [
        os.path.join(ROOT_DIR, "models", "face_landmarker.task"),
        os.path.join(ROOT_DIR, "models", "face_landmarker_v2_with_blendshapes.task"),
    ]
    for path in candidates:
        if os.path.isfile(path):
            return path
...
def load_resource_images(prefix, flags=cv2.IMREAD_COLOR):
    """!Load resources from the resources directory by name prefix.

    @param prefix File prefix (e.g. \"background\" or \"face\").
    @param flags cv2.imread flags.
    @return List of (path, image) tuples.
    """
    patterns = [
...
def init_controls(bg_count, face_count):
    """!Create trackbars for live tuning.

    @param bg_count Number of available background images.
    @param face_count Number of available face images.
    """
    ...
    cv2.createTrackbar("FaceScale", "Controls", DEFAULT_FACE_SCALE, 220, lambda _: None)
    cv2.createTrackbar("Mouth", "Controls", DEFAULT_MOUTH, 1, lambda _: None)
...
def main():
    """!Entry point for the real-time augmentation app."""
    ...
    face_landmarker_path = default_face_landmarker_path()
    face_replacer = FaceReplacer(
        refresh=6,
        min_size=60,
        landmarker_model_path=face_landmarker_path,
        landmark_refresh=3,
    )
    ...
    try:
        while True:
            ...
            if face_img is not None:
                face_scale = cv2.getTrackbarPos("FaceScale", "Controls") / 100.0
                if face_scale < 0.8:
                    face_scale = 0.8
                    cv2.setTrackbarPos("FaceScale", "Controls", int(face_scale * 100))
                mouth_enabled = cv2.getTrackbarPos("Mouth", "Controls") == 1
                face_box, mouth_open = face_replacer.detect(frame, frame_idx, need_mouth=mouth_enabled)
                if not mouth_enabled:
                    mouth_open = None
                output = face_replacer.apply(
                    output,
                    face_img,
                    face_box,
                    scale=face_scale,
