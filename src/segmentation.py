...
class PersonSegmenter:
    """!Real-time person segmentation with optional MediaPipe backend selection.

    Wraps MediaPipe segmentation (solutions or tasks API) and applies
    temporal smoothing + post-processing to produce a stable mask.
    """
    ...
    def __init__(
        ...
        """!Initialize the segmenter with post-processing parameters.

        @param model_selection MediaPipe selfie model index (solutions API).
        @param threshold Mask threshold in [0, 1].
        @param smooth Temporal smoothing factor in [0, 1].
        @param blur Blur radius for mask smoothing (0 disables).
        @param morph Morphology radius for open/close (0 disables).
        @param model_path TFLite model path for MediaPipe Tasks API.
        """
        if mp is None:
            raise RuntimeError(
    ...
    def __call__(self, frame_bgr):
        """!Compute person mask for a single BGR frame.

        @param frame_bgr Input frame in BGR format (uint8).
        @return Tuple (mask_soft, mask_bin) where mask_soft is float32 [0..1]
                and mask_bin is uint8 {0,1}.
        """
